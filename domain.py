...
class DiscreteRoverDomain:
    def __init__(self, p):
        ...
        self.n_rooms = len(p.rooms)                     # number of rooms
        self.rooms = p.rooms

        self.n_poi_types = np.shape(self.rooms[0])[0]   # how many types of POIs - allows for calc of L
    ...
    def gen_world(self):
        ...
        self.rooms.append(hallway_poi)
        p = [self.p for _ in range(len(self.rooms))]
        idxs = [i for i in range(len(self.rooms))]

        pois = []
        for i, poi_ct in enumerate(self.rooms):
    ...
    def step(self, actions):
        ...
        for i, ag in enumerate(self.agents):
            if actions[i]:
                ag.goal = actions[i]  # agents set a new goal at every time step
    ...
    def action(self, agent, nn_output):
        ...
        # If the room number (index) == number of rooms, then the agent is in the hallway (last room added on beyond room count)
        # If in the hallway, go to door of room chosen
        elif agent.curr_rm == self.n_rooms:
            goal = self.rooms[rm_num].door

        # If in a different room, go to door of the current room
        else:
            goal = self.rooms[agent.curr_rm].door
